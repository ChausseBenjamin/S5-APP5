...
def _():
    mo.md(r"""
    ### i) Le pouvoir est obtenu si toutes les roues s'arrêtent sur le même pictogramme. Deux configurations sont testés:
...
def _():
    mo.md(r"""
    ### ii) C'est le cas où un pouvoir est obtenue si toutes les roues affichent des pictogrammes différents.
...
def _():
    ...
    return dataset, hours, stats_dict
...
def _(dataset):
    ...
    return centres, classes_col, freq_cumulees, freq_relatives, k, limites


@app.cell(hide_code=True)
def _(centres, classes_col, freq_cumulees, freq_relatives, limites):
...
def _(dataset):
    ...
    while _i < len(_expected_freq_raw):
        ...
        observed_freq.append(_obs_merged)
        expected_freq.append(_exp_merged)
        bins_edges.append((_lower_merged, _upper_merged))
        _i += 1
    ...
    ddof_chi2 = len(observed_freq) - 1 - 2
...
@app.cell(hide_code=True)
def _(bins_edges, expected_freq, observed_freq):
    chi2_result_df = pl.DataFrame(
        {
            "Classe": [
                f"[{_lower:.0f}, {_upper:.0f}]"
                for _lower, _upper in bins_edges
            ],
            "Freq. observee": observed_freq.tolist(),
            "Freq. attendue": [round(_f, 2) for _f in expected_freq],
        }
    )
...
def _(alpha_v, mu_0, n, s, x_bar):
    ...
    # Power of the test
    power = 1 - beta
    return beta, mu_true, power, se, x_critical, z_beta


@app.cell(hide_code=True)
def _(beta, mu_0, mu_true, power, se, x_critical, z_beta):
